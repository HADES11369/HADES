...
def create_shm_mem(shm_name, malloc_size, pin_memory=True):
    ptr, fd = EmbCacheLib.create_shared_mem(shm_name, malloc_size, pin_memory)
    return ptr, fd


def release_shm_mem(shm_name, shm_size, ptr, fd, pin_memory=True):
    EmbCacheLib.release_shared_mem(shm_name, shm_size, ptr, fd, pin_memory)


def open_shm_mem(shm_name, shm_size, pin_memory=True):
    ptr, fd = EmbCacheLib.open_shared_mem(shm_name, shm_size, pin_memory)
    return ptr, fd


def open_shared_tensor(ptr, dtype, shape):
    tensor = EmbCacheLib.open_shared_tensor(ptr, dtype, shape)
    return tensor
